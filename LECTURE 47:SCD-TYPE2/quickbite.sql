...
CREATE OR REPLACE STAGE RAW.S3_BATCH_STAGE
  ...
  URL = 's3://quickbite-lake/raw/'
  FILE_FORMAT = RAW.QUICKBITE_CSV_FORMAT;
...
FROM (
  SELECT
    ...
    METADATA$FILENAME,
    CURRENT_TIMESTAMP()
  FROM '@RAW.S3_BATCH_STAGE/customers/' (FILE_FORMAT => 'QUICKBITE_CSV_FORMAT') t
)
ON_ERROR='CONTINUE';
...
FROM (
  SELECT t.$1::STRING, t.$2::STRING, t.$3::STRING, TRY_TO_DOUBLE(t.$4), TRY_TO_DOUBLE(t.$5), METADATA$FILENAME, CURRENT_TIMESTAMP()
    FROM '@RAW.S3_BATCH_STAGE/restaurants/' (FILE_FORMAT => 'QUICKBITE_CSV_FORMAT') t
)
ON_ERROR='CONTINUE';
...
FROM (
  SELECT t.$1::STRING, t.$2::STRING, t.$3::STRING, t.$4::STRING, t.$5::STRING, METADATA$FILENAME, CURRENT_TIMESTAMP()
    FROM '@RAW.S3_BATCH_STAGE/drivers/' (FILE_FORMAT => 'QUICKBITE_CSV_FORMAT') t
)
ON_ERROR='CONTINUE';
...
FROM (
  SELECT 
    ...
    METADATA$FILENAME,
    CURRENT_TIMESTAMP()
    FROM '@RAW.S3_BATCH_STAGE/menu_items/' (FILE_FORMAT => 'QUICKBITE_CSV_FORMAT') t
)
ON_ERROR='CONTINUE';
...
FROM (
  SELECT t.$1::STRING, TRY_TO_DOUBLE(t.$2), TRY_TO_DOUBLE(t.$3), TRY_TO_DATE(t.$4), TRY_TO_DATE(t.$5), METADATA$FILENAME, CURRENT_TIMESTAMP()
    FROM '@RAW.S3_BATCH_STAGE/coupons/' (FILE_FORMAT => 'QUICKBITE_CSV_FORMAT') t
)
ON_ERROR='CONTINUE';
...
FROM (
  SELECT
    ...
    METADATA$FILENAME,
    CURRENT_TIMESTAMP()
    FROM '@RAW.S3_BATCH_STAGE/orders/' (FILE_FORMAT => 'QUICKBITE_CSV_FORMAT') t
)
ON_ERROR='CONTINUE';
...
FROM (
  SELECT
    ...
    METADATA$FILENAME,
    CURRENT_TIMESTAMP()
  FROM '@RAW.S3_BATCH_STAGE/order_items/' (FILE_FORMAT => 'QUICKBITE_CSV_FORMAT') t
)
ON_ERROR='CONTINUE';
...
SELECT * FROM DEMO_SCD2.AUDIT.PIPELINE_JOB_LOG;

CREATE OR REPLACE PROCEDURE QSTG.SP_PROCESS_RAW_ORDERS()
RETURNS VARIANT
LANGUAGE JAVASCRIPT
EXECUTE AS CALLER
AS
