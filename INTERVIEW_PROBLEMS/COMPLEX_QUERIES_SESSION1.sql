...
*/
-- getting only the products who have been ordered 

-- USING THIS QUERY I got the top 5 customer ids
SELECT CUSTOMER_ID
FROM (
    SELECT
        T1.CUSTOMER_ID, 
        DENSE_RANK() OVER(ORDER BY SUM(T1.QUANTITY * T2.UNIT_PRICE) DESC) AS RANKING_OF_CUSTOMERS 
    FROM ORDERS AS T1
    INNER JOIN PRODUCTS AS T2
    ON T1.PRODUCT_ID = T2.PRODUCT_ID
    GROUP BY T1.CUSTOMER_ID
)
WHERE RANKING_OF_CUSTOMERS <= 5;

-- I need to get the information of the above customers only 
SELECT
    T5.CUSTOMER_ID,
    T5.CUSTOMER_NAME, 
    COUNT(T3.ORDER_ID) AS TOTAL_ORDERS, 
    SUM(T3.QUANTITY * T4.UNIT_PRICE) AS TOTAL_AMOUNT_SPENT
FROM ORDERS AS T3
INNER JOIN PRODUCTS AS T4
ON T3.PRODUCT_ID = T4.PRODUCT_ID
INNER JOIN CUSTOMERS AS T5
ON T3.CUSTOMER_ID = T5.CUSTOMER_ID
WHERE T5.CUSTOMER_ID IN (
        -- returns the top 5 customer IDs
        SELECT CUSTOMER_ID
        FROM (
            SELECT
                T1.CUSTOMER_ID, 
...
GROUP BY T5.CUSTOMER_ID ,T5.CUSTOMER_NAME;
...
SELECT
    T2.PRODUCT_NAME, 
    T2.CATEGORY, 
    SUM(T1.QUANTITY) AS TOTAL_QUANTITY_ORDERED
FROM ORDERS AS T1
INNER JOIN PRODUCTS AS T2
ON T1.PRODUCT_ID = T2.PRODUCT_ID
GROUP BY T2.PRODUCT_NAME, T2.CATEGORY
HAVING SUM(T1.QUANTITY) >
    (
        SELECT ROUND(AVG(TOTAL_QUANTITIY_S1),0) AS AVERAGE_QUANITTY
        FROM 
        (
            -- Inner query is returning me the products and the total quantity ordered for that products 
            -- Reason to do this, to get the correct denominator for AVG()
            SELECT 
                PRODUCT_ID,  
                SUM(QUANTITY) AS TOTAL_QUANTITIY_S1
            FROM ORDERS 
            GROUP BY PRODUCT_ID 
        )
);
...
SELECT
    T1.CUSTOMER_ID, 
    T1.CUSTOMER_NAME, 
    COUNT(T2.ORDER_ID) AS COUNT_OF_ORDERS
FROM CUSTOMERS AS T1
-- TO GET INFORMATION ABOUT ALL THE CUSTOMERS 
LEFT JOIN ORDERS AS T2
ON T1.CUSTOMER_ID = T2.CUSTOMER_ID
-- TO GET ONLY THE PRODUCTS THAT WERE ORDERED
INNER JOIN PRODUCTS AS T3
ON T2.PRODUCT_ID = T3.PRODUCT_ID
-- CONDITION THAT WILL FILTER THE GAMING CATEGORY
WHERE T3.CATEGORY != 'Gaming'
...
WHERE T1.CUSTOMER_ID NOT IN (
        SELECT DISTINCT TS1.CUSTOMER_ID
        FROM ORDERS AS TS1
        JOIN PRODUCTS AS TS2
        ON TS1.PRODUCT_ID = TS2.PRODUCT_ID
        WHERE TS2.CATEGORY != 'Gaming'
    )
GROUP BY T1.CUSTOMER_ID, T1.CUSTOMER_NAME;
