...
def generate_cte_for_LockInStreamOut(xml_data, previousToolId, toolId, prev_tool_fields):
    root = ET.fromstring(xml_data)

    isSorted = root.find('.//Configuration/Sort').get('value')
    ...
    if isSorted =='True':
        ...
        cte_query = f"""
                CTE_{toolId} AS (
                    SELECT
                        {', '.join([f'"{field}"' for field in prev_tool_fields])}
                    FROM CTE_{previousToolId}
                )
                """
...
def generate_cte_for_DbFileOutput(xml_data, previousToolId, toolId, prev_tool_fields):
...
def sanitize_expression_for_filter_formula_dynamic_rename(expression, field_name=None):
    ...
    # Ensure TO_CHAR() and TO_DATE() fields are properly enclosed in single quotes
    def fix_to_char_to_date(match):
        function_name = match.group(1) # Extrcats TO_CHAR or TO_DATE
        field = match.group(2).strip()
        format_string = match.group(3).strip()

        # If field is already in double quotes, convert to single quotes
        if field.startswith('"') and field.endswith('"'):
            field = f"'{field[1:-1]}'"

        # If field is already in single quotes, keep it unchanged
        elif field.startswith("'") and field.endswith("'"):
            pass

        # If field is NOT in any quotes, add single quotes
        else:
            field = f"'{field}'"
...
def generate_cte_for_AppendFields(xml_data, sourceToolID, targetToolID, toolId, sourceToolColList, TargetToolColList):
    ...
    CTE_{toolId} AS (
        SELECT 
            {', '.join(final_fields)}
        FROM CTE_{targetToolID} TargetTable
        CROSS JOIN CTE_{sourceToolID} SourceTable
    )
    """
    return current_tool_fields, cte_query
